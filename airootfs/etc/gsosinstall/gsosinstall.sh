...
xargs -0 zenity --list --width=600 --height=512 --title="Select disk" --text="Select your disk to install GuestSneezeOS in below:\n\n $(lsblk -d -o NAME,MAJ:MIN,RM,SIZE,RO,TYPE,VENDOR,MODEL,SERIAL,MOUNTPOINT)" \
--radiolist --multiple --column ' ' --column 'Disks')
	...
	if [[ -n "$(sudo blkid | grep holo-home | cut -d ':' -f 1 | head -n 1)" ]]; then
		HOME_REUSE_TYPE=$(zenity --list --title="Warning" --text="GuestSneezeOS home partition was detected at $(sudo blkid | grep holo-home | cut -d ':' -f 1 | head -n 1). Please select an appropriate action below:" --column="Type" --column="Name" 1 "Format it and start over" \2 "Reuse partition"  --width=500 --height=220)
	...
	case $install in
		1)
			...
			;;
		2)
			echo "\nGuestSneezeOS will be installed alongside existing OS/Partition.\nPlease make sure there are more than 24 GB on the >>END<< of free(unallocate) space available\n"
			parted $DEVICE print free
			echo "GuestSneezeOS will be installed on the following free (unallocated) space.\n"
			...
			if [ $? != 0 ]; then
				...
				$INST_MSG1
			if zenity --question --text "GuestSneezeOS will be installed on the following free (unallocated) space.\nDoes this look reasonable?\n$(sudo parted ${DEVICE} print free | tail -n2 | grep "Free Space")" --width=500
			then
...
base_os_install() {
	...
	arch-chroot ${HOLO_INSTALL_DIR} rm /etc/polkit-1/rules.d/99_GuestSneezeOS_installuser.rules
	cp -r /etc/gsosinstall//post_install/pacman.conf ${HOLO_INSTALL_DIR}/etc/pacman.conf
	arch-chroot ${HOLO_INSTALL_DIR} pacman-key --init
    arch-chroot ${HOLO_INSTALL_DIR} pacman -Rdd --noconfirm $(cat /etc/gsosinstall//post_install/kernel_list.bootstrap)
	arch-chroot ${HOLO_INSTALL_DIR} mkinitcpio -P
    arch-chroot ${HOLO_INSTALL_DIR} pacman -U --noconfirm $(find /etc/gsosinstall//post_install/kernels | grep pkg.tar.zst)
	...
	mv /etc/gsosinstall//post_install_shortcuts/steam.desktop ${HOLO_INSTALL_DIR}/etc/xdg/autostart
    mv /etc/gsosinstall//post_install_shortcuts/steamos-gamemode.desktop ${HOLO_INSTALL_DIR}/etc/skel/Desktop	
	...
	arch-chroot ${HOLO_INSTALL_DIR} GuestSneezeOS-grub-update
	mount -o remount,rw -t efivarfs efivarfs /sys/firmware/efi/efivars
	arch-chroot ${HOLO_INSTALL_DIR} efibootmgr -c -d ${DEVICE} -p ${efiPartNum} -L "GuestSneezeOS" -l '\EFI\BOOT\BOOTX64.efi'
	sleep 1
	clear
}
full_install() {
	if [[ "${GAMEPAD_DRV}" == "1" ]]; then
		...
		arch-chroot ${HOLO_INSTALL_DIR} pacman -U --noconfirm $(find /etc/gsosinstall//post_install/pkgs_addon | grep win600-xpad-dkms)
	fi
	if [[ "${FIRMWARE_INSTALL}" == "1" ]]; then
		...
		arch-chroot ${HOLO_INSTALL_DIR} pacman -U --noconfirm $(find /etc/gsosinstall//post_install/pkgs_addon | grep linux-firmware-neptune)
		arch-chroot ${HOLO_INSTALL_DIR} mkinitcpio -P
	fi

	mv /etc/gsosinstall//post_install/amd-perf-fix "${HOLO_INSTALL_DIR}"/usr/bin/amd-perf-fix
	chmod +x "${HOLO_INSTALL_DIR}"/usr/bin/amd-perf-fix
	
	# if [[ -n "$(lspci -nn | grep -i vga | grep -Po "10de:[a-z0-9]{4}")" ]]; then
	# 	echo "NVIDIA GPU detected. Installing NVIDIA Drivers."
	# 	arch-chroot ${HOLO_INSTALL_DIR} pacman -U --noconfirm $(find /etc/gsosinstall//post_install/pkgs/nv | grep pkg.tar.zst)
	# 	arch-chroot ${HOLO_INSTALL_DIR} mkinitcpio -P
	# fi
	...
	arch-chroot ${HOLO_INSTALL_DIR} touch /etc/gsosinstall/-oobe
	echo "Cleaning up..."
	cp /etc/skel/.bashrc ${HOLO_INSTALL_DIR}/home/${HOLOUSER}
    arch-chroot ${HOLO_INSTALL_DIR} rm -rf /etc/gsosinstall/
...
HOLO_INSTALL_TYPE=$(zenity --list --title="Choose your installation type:" --column="Type" --column="Name" 1 "Install GuestSneezeOS, version $(cat /etc/os-release | grep VARIANT_ID | cut -d "=" -f 2 | sed 's/"//g') " \2 "Exit installer"  --width=700 --height=220)
